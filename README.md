...
- C ソース（`.c`）/ C++ ソース（`.cpp`）から関数定義を直接抽出
- **複数ファイルを同時指定して1つの出力にマージ**（重複定義は自動スキップ）
- 生成コードの構文検証（`--validate`）
...
```
c2ffidef [OPTIONS] <file> [<file2> ...]

入力ファイルの種類:
  .h              C ヘッダ         — 全宣言を出力
  .hpp / .hh      C++ ヘッダ       — extern "C" 宣言のみを出力
  .c              C ソース         — 当該ファイルの関数定義を出力
                                     インクルードされたヘッダの型定義も出力
  .cpp / .cxx     C++ ソース       — extern "C" 関数定義のみを出力

複数ファイルは個別に解析されたあと1つの出力にマージされます。
型名・関数名が重複する場合は最初の出現が採用されます（first wins）。
...
Options:
  ...
  --no-system    システムヘッダの宣言を除外（デフォルト: 除外）
...
# 複数の C ソースを渡して1つの Python バインディングにマージ
./build/c2ffidef -l libmylib.so -t python src/foo.c src/bar.c -o bindings.py

# C ソースと C++ ソースを混在させることも可能
./build/c2ffidef -l libmylib.so src/foo.c src/bar.cpp -o ffi_out.php
...
ヘッダファイルだけでなく、`.c` / `.cpp` ソースファイルを直接渡すことができます。
ソースファイルでは**関数定義**が対象になります（宣言のみの `.h` とは異なります）。

| ファイル種別 | 抽出される関数 | 型定義の範囲 |
|------------|--------------|------------|
| `.c` | 当該ファイルの関数定義（`static` は除外） | インクルードされたヘッダも含む |
| `.cpp` | `extern "C"` ブロック内の関数定義のみ | インクルードされたヘッダも含む |

```c
// foo.c  — static は出力されない、extern な関数定義だけ抽出
#include "mylib.h"

static int internal_helper(int x) { return x * 2; }  // → 出力されない

int mylib_add(int a, int b) { return a + b; }         // → 出力される
int mylib_sub(int a, int b) { return a - b; }         // → 出力される
```

```cpp
// bar.cpp  — extern "C" 指定のものだけ抽出
namespace detail { class Impl {}; }  // → 無視

extern "C" {
    void mylib_init(void) {}         // → 出力される
    void mylib_cleanup(void) {}      // → 出力される
}

extern "C" int mylib_version(void) { return 1; }  // → 出力される
float cpp_only(float x) { return x; }             // → 出力されない
```

```bash
./build/c2ffidef -l libmylib.so -t python foo.c bar.cpp -o bindings.py
```
...
複数のファイルを同時に渡すと、それぞれ独立して解析されたあと1つの出力にマージされます。
同じ名前の型・関数が複数のファイルに存在する場合は**最初に現れたもの**が採用されます。

```bash
# 複数 C ソース → 1 つの PHP バインディング
./build/c2ffidef -l libfoo.so -t php src/audio.c src/video.c src/codec.c -o ffi.php

# ヘッダと C++ ソースを混在
./build/c2ffidef -l libfoo.so -t python api.h impl.cpp -o ctypes_bindings.py

# 同じファイルを2回渡しても重複しない
./build/c2ffidef -t python foo.c foo.c   # → 重複なし
```

進捗は標準エラーに出力されます（リダイレクトで抑制可能）：
...
├── testlib.c           — C ソースファイルの抽出テスト（static 除外）
└── testlib.cpp         — C++ ソースファイルの抽出テスト（extern "C" のみ）
```
...
- 複数ファイルのマージ時、同名の型・関数は最初のファイルのものが採用されます（後から来たものは無視）
- `.c` / `.cpp` ソースファイルを渡す場合、リンケージが `static` な関数は出力されません
